...
describe('aside', () => {
  describe('standard', async () => {
    document.body.innerHTML = await readFile({ path: './mocks/standard.html' });
    ...
    describe('default', () => {
      ...
      it('allows a background color', () => {
        expect(el.style.backroundColor).to.equal('#eeeeee');
      });
      ...
      it('allows a foreground video', () => {
        expect(el2.querySelector('.foreground .image video')).to.exist;
      });
    });
